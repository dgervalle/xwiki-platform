...
public class XWikiMigrationManagerTest extends AbstractBridgedXWikiComponentTestCase
{
    ...
    private void registerComponent(Class<?> klass) throws Exception
    {
        ...
        System.out.println(descriptors.get(0).getImplementation().getName());
        System.out.println(descriptors.get(0).getRole().getName());
        System.out.println(descriptors.get(0).getRoleHint());
        for (ComponentDescriptor<?> descriptor : descriptors) {
            getComponentManager().registerComponent(descriptor);
        }

        new TestDataMigrationManager();
    }
    ...
    public void testMigrationForce() throws Exception
    {
        ...
        assertEquals(
            567,
            ((AbstractDataMigrationManager.XWikiMigration) neededMigration.toArray()[0]).migrator.getVersion().getVersion());
    }
}
